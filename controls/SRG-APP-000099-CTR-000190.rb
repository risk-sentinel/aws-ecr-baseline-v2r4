control 'SRG-APP-000099-CTR-000190' do
  ...
  desc 'Within the container platform, audit data can be
                ...
                is necessary to know the outcome of the
                event.'
  desc 'check', 'Review the container platform configuration to determine if audit
                    records contain the audit event results. Generate audit records and review the
                    data to validate that the record does contain the event result. If the container
                    platform is not configured to generate audit records with the event result or
                    the audit record does not contain the event result, this is a finding.'
  desc 'fix', 'Configure the container platform to generate audit
                records that contain the event result.'
  impact 0.5
  tag check_id: 'C-35982r601628_chk'
  tag severity: 'medium'
  tag gid: 'V-233046'
  tag rid: 'SV-233046r960903_rule'
