control 'SRG-APP-000391-CTR-000935' do
  title 'The container platform must be configured to use multi-factor authentication for
                user authentication.'
  desc 'Controlling access to the container platform and its
                ...
                container platform or they may be validated by each component. To standardize and
                reduce the risks of unauthorized access, the use of multifactor token-based
                credentials is the preferred method. DoD has mandated the use of the CAC to support
  ...
  desc 'check', 'Review documentation and configuration to ensure the container
                    platform is configured to use an approved DoD multifactor token (CAC) when
                    accessing platform via user interfaces. If multifactor authentication is not
                    configured, this is a finding.'
  desc 'fix', 'Configure the container platform to accept standard
                DoD multifactor token-based credentials when users interface with the platform.'
  impact 0.5
